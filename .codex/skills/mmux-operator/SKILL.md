...
1. Confirm the mmux server:
   ...
   operators must not assume every built-in profile is available. When a tool
   omits `profile`, mmux uses `--default-coder-profile` if configured,
   otherwise the first enabled built-in in canonical order: `codex`,
