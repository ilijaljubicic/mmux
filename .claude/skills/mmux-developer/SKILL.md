...
- `.codex/skills/mmux-*` and `.claude/skills/mmux-*` skills and
  `references/mcp-recipes.md`; keep matching copies synchronized.
- Installed mmux skills in default and project-specific Codex/Claude homes
...
- `start_coding_session` with a valid `task_id` when testing session creation;
  `session_record` with a valid task for an existing manually created session.
- Missing/invalid-task launches and `exec` against missing/unrecorded sessions
  must fail without creating sessions. `exec` never creates a session.
